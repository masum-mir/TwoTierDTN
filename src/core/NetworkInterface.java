...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	/** radio group -setting id ({@value})*/
	public static final String RADIO_GROUP_S = "radioGroup";
	...
	public NetworkInterface(Settings s) {
		...
		this.radioGroup = s.contains(RADIO_GROUP_S) ? s.getSetting(RADIO_GROUP_S)
				: this.interfacetype;
	...
	public NetworkInterface() {
		...
		this.connections = new ArrayList<Connection>();
		this.radioGroup = this.interfacetype;
	}
	...
	public void setHost(DTNHost host) {
		...
		if (transmitRange > 0) {
			optimizer = ConnectivityGrid.ConnectivityGridFactory(
					this.radioGroup.hashCode(), transmitRange);
	...
	/**
	 * For checking what radio group this interface belongs to
	 */
