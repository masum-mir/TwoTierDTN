 ...
 * 
 */
public class TwoTierRouterAdvanced extends ActiveRouter {

    // ── Settings keys ────────────────────────────────────────────────────────

    /** Router namespace in default_settings.txt */
    public static final String ROUTER_NS = "TwoTierRouterAdvanced";
    ...
    public TwoTierRouterAdvanced(Settings s) {
        super(s);
    ...
    protected TwoTierRouterAdvanced(TwoTierRouterAdvanced r) {
        super(r);
    ...
    // ── Connection event: update drone encounter probability ──────────────────
    ...
    // ── Buffer eviction: custody protection ──────────────────────────────────
    ...
    // ── Transfer done: delete-after-upload for nodes ──────────────────────────
    ...
    // ── Message received: start custody + mark immunity ───────────────────────
    ...
    private void tryForwardStrict(List<Message> messages,
        ...
        for (Connection con : connections) {
            if (!con.isUp()) continue;
            DTNHost peer = con.getOtherNode(getHost());
            if (!isAllowedRelay(getHost(), peer)) continue;
    ...
    private boolean isAllowedRelay(DTNHost from, DTNHost to) {
    ...
    // ── Routing info for ONE Simulator GUI ────────────────────────────────────
    ...
    @Override
    public TwoTierRouterAdvanced replicate() {
        return new TwoTierRouterAdvanced(this);
    }
}
