...
public class EnergyModel implements ModuleCommunicationListener {
    ...
    public static final String MOVEMENT_ENERGY_S = "movementEnergy";
    public static final String MOVEMENT_ENERGY_ID = "Energy.movement";

    public static final String INITIAL_ENERGY_ID  = "Energy.initial";
    ...
    private double movementConsumed; 
	...
	public EnergyModel(Settings s) {
		...
		this.movementEnergy = s.getDouble(MOVEMENT_ENERGY_S);
	...
	protected EnergyModel(EnergyModel proto) {
        ...
        this.movementEnergy     = proto.movementEnergy;
        this.movementConsumed   = 0; 
        this.lastMovementUpdate = 0;
    ...
    /**  Returns initial energy (midpoint if range). */
    public double getInitialEnergy() {
        return initEnergy.length == 1 ? initEnergy[0]
                                      : (initEnergy[0] + initEnergy[1]) / 2.0;
    }
 
    /**  Returns movement energy rate (J/s). */
    public double getMovementRate() {
        return this.movementEnergy;
    }
    
    public double getMovementConsumed() { 
    	return this.movementConsumed; 
    }

    //  initComBus 
    
    public void initComBus(ModuleCommunicationBus comBus) {
        if (this.comBus != null) return;
        this.comBus = comBus;
        try {
        	this.comBus.addProperty(ENERGY_VALUE_ID,   this.currentEnergy);
            this.comBus.addProperty(INITIAL_ENERGY_ID, this.getInitialEnergy());  
            this.comBus.addProperty(MOVEMENT_ENERGY_ID, 0.0); 
        }catch (SimError e) {
        	this.comBus.updateProperty(ENERGY_VALUE_ID,   this.currentEnergy);
            this.comBus.updateProperty(INITIAL_ENERGY_ID, this.getInitialEnergy());  
            this.comBus.updateProperty(MOVEMENT_ENERGY_ID, 0.0); 
        }
	...
	public void updateMovementEnergy(double simTime, boolean isMoving) {
        if (this.movementEnergy == 0.0) return; 
        if (simTime < this.warmupTime)  return; 
        if (this.comBus == null)        return;  
 
        double delta = simTime - this.lastMovementUpdate;
        if (delta <= 0) return;
 
        if (isMoving) {
            double cost = this.movementEnergy * delta;
            reduceEnergy(cost);
            this.movementConsumed += cost;
            // update comBus so EnergyLevelReport can read it
            this.comBus.updateProperty(MOVEMENT_ENERGY_ID,
                    this.movementConsumed);
        }
 
        this.lastMovementUpdate = simTime;
    }
	
	public boolean isOutOfEnergy() {
	    return this.currentEnergy <= 0.0;
	}
}